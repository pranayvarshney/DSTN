...
#
# python disk_1.py -a 17,28,8,31,11,30,12,20,32,24,25,6,4,23,3,15,10,21,22,33,18,5,27,29,6,8,11,2,13,19,14,34,7,9,1,16,26 -p CSCAN -G -w 5
class Disk:
    ...
    def DoSCAN(self,rList):
        ...
        for (block, index) in rList:
            ...
            if self.currentTrack  ==-1:
                ...
                trackList = []
                trackList.append((block, index))
            elif self.currentTrack == track:
                trackList.append((block, index))

        # print("tracklist",trackList)
        if(trackList==[]):
            self.currentTrack=(self.currentTrack+1)%3;
            return self.DoSCAN(rList)

        assert(trackList != [])
        return trackList
        # currentTrack = self.armTrack
        # print(currentTrack)
        # returnList = []
        # for(block,index) in rList:
        #     if(self.blockToTrackMap[block]==currentTrack):
        #         returnList.append((block,index))

        # if(returnList!=[]):
        #     return returnList
        # else:
        #     self.armTrack = (self.armTrack + 1)%2
        #     return self.DoSCAN(rList)
    ...
    def GetNextIO(self):
        ...
        if self.policy == 'FIFO':
            ...
            print("tracklistreceived",trackList)
...
parser.add_option('-p', '--policy',          default='FIFO',      help='Scheduling policy (FIFO, SSTF, SATF, BSATF, SCAN)',      action='store', type='string', dest='policy')

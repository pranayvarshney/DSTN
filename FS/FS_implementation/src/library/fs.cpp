...
void FileSystem::debug(Disk *disk)
{
    ...
    {
        // counter i is inode block counter
        disk->read(i + 1, inodeBlock.Data);
        for (uint32_t j = 0; j < INODES_PER_BLOCK; j++)
        {
            // counter j is jth inode in an ith inode block
            if (inodeBlock.Inodes[j].Valid && inodeBlock.Inodes[j].Size>0)
            {
                // the current inode number
                printf("Inode %d:\n", j + i * INODES_PER_BLOCK);
                // file size of the file in jth inode
                printf("    size: %d bytes\n", inodeBlock.Inodes[j].Size);

                string directBlockString = "    direct blocks:";

                for (uint32_t k = 0; k < POINTERS_PER_INODE; k++)
                {
                    if (inodeBlock.Inodes[j].Direct[k])
                    {
                        // if direct pointer of an inode is not empty print the pointer it has
                        directBlockString += " ";
                        directBlockString += to_string(inodeBlock.Inodes[j].Direct[k]);
                    }
                }
                cout << directBlockString << endl;

                // if indirect pointer of an inode is not empty proceed the same according to above logic
                if (inodeBlock.Inodes[j].Indirect)
                {
                    ...
                    string indiString = "    indirect data blocks:";
                    ...
                    for (uint32_t k = 0; k < POINTERS_PER_BLOCK; k++)
                    {
                        if (indiBlock.Pointers[k])
                        {
                            ...
                            indiString += " ";
                            indiString += to_string(indiBlock.Pointers[k]);
                        }
                    }

                    cout << indiString << endl;
                }
            }
        }
    }
...
ssize_t FileSystem::read(size_t inumber, char *data, size_t length, size_t offset)
{
    ...
    // Adjust length
    length = min(length, loadedInode.Size - offset);
    ...
    while (startBlock < POINTERS_PER_INODE)
    {
        ...
        uint32_t dataSize = min(startByte + readIndex, blockSizeVar);
